...
class TestFeedEntry:
    ...
    def test_feed_entry_str_representation(self) -> None:
        ...
        assert str(entry) == "FeedEntry(title='Article', url='https://example.com', author='Author', published='2023-01-01')"
...
class TestParseFeed:
    ...
    def test_parse_feed_with_numeric_urls(self, mock_parse) -> None:
        ...
        mock_feed.entries = [
            Mock(
                link="12345",
                title="Article with numeric URL"
            ),
            Mock(
                link="https://example.com/67890",
                title="Article with mixed URL"
            )
        ]
        mock_parse.return_value = mock_feed
